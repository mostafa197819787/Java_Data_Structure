public class ARDeque<T> {
    ...
    */
    ...
    private void resize (int capacity) {
        ...
        int curr = plusOne(nextFirst); // index = 0;
        for (int i = 0; i < size; i++) {
        ...
        items = newArray;
        nextFirst = capacity - 1; // 对于array中的null，nextFirst指向最后一位，nextLast指向第一位
        nextLast = size;
    }
    ...
    */
    ...
    public ARDeque() {
        items = (T[]) new Object[4];
        size = 0;
        nextFirst = 0; // null deque, 第一位就是null
        nextLast = 1; // initialize, nextLast = nextFirst + 1
    }
    ...
    public T get (int index) {
        if (size == 0 || index < 0 || index >= size)
            throw new IndexOutOfBoundsException("Index " + index + " is not valid");
        index = (plusOne(nextFirst) + index) % items.length; // find real index, basically index of nextFirst is the last one in array
        return items[index];
    }
    ...
    /*public ARDeque(ARDeque<T> other) {
        ...
        this.resize(other.items.length);
