public class WeightedQuickUnion {
    ...
    /*public WeightedQuickUnion(int N) {
        ...
        parent = new int[N]; // To initialize parent
    ...
    /*public void validate(int p) {
        ...
        if(p >= parent.length || p < 0){ // when p is invalidate
    ...
    /*public int sizeOf(int p) {
        ...
        int root = find(p);
		return Math.abs(parent[root]); // size is positive
    ...
    public void union(int p, int q) {
        if (!isSameGroup(p, q)) {
            int root_p = find(p);
            int root_q = find(q);
            if (sizeOf(root_p) <= sizeOf(root_q)) {
                parent[root_p] = root_q;
                parent[root_q] -= sizeOf(p);
            }
            else {
