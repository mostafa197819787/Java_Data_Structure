...
public class OddAndTen {
    ...
    private static boolean oddAndTenHelper(List<Integer> list, int start, int sum1, int sum2) { // add any parameters
        ...
        return (oddAndTenHelper(list, start + 1, sum1 + list.get(start), sum2)
                || oddAndTenHelper(list, start + 1, sum1, sum2 + list.get(start)));
    ...
    private static boolean oddAndTenHelper (List<Integer> list, int start, int sum1, int sum2) {
        if (start == list.size()){
            if ((sum1 % 10 == 0 && sum2 % 2 == 1) || (sum1 % 2 == 1 && sum2 % 10 == 0))
                return true;
            else
                return false;
        }
        else{
            return oddAndTenHelper(list, start + 1, sum1 + list.get(start), sum2) || oddAndTenHelper(list, start + 1, sum1, sum2 + list.get(start));
        }
    }
}
