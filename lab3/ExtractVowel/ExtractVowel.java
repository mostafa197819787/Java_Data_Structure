public class ExtractVowel {
    ...
    private static String extractVowelHelper(String str, int start, String vowels) {
        ...
        if (isVowel(str.charAt(start)))
            return str.charAt(start) + extractVowelHelper(str, start + 1, vowels + str.charAt(start));
    ...
    private  static String extractVowelHelper (String str, int start, String vowels) {
        if(start == str.length())
            ...
            if (isVowel(str.charAt(start)))
                return str.substring(start, start + 1) + extractVowelHelper(str, start + 1, vowels + str.charAt(start));
            else
                return extractVowelHelper(str, start + 1, vowels);
        }
    }
